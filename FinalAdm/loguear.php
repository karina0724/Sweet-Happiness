...
if($array['contar']>0){
    ...
    $q = "SELECT nombre, apellido, rol FROM administrador where cedula = '$usuario'";
    $consulta = mysqli_query($conexion, $q);
...
header("location: index.php");
    ...
    $_SESSION['initialization_sistem'] = true;
    $q = "SELECT nombre, apellido, rol FROM reservadores_huesped where cédula = '$usuario'";
    $consulta = mysqli_query($conexion, $q);
    $arr = mysqli_fetch_array($consulta);
    $_SESSION['username'] = $arr['nombre'];
    $_SESSION['rol'] = $arr['rol'];
...
header("location: index.php");
    ...
    $_SESSION['initialization_sistem'] = true;
    $q = "SELECT nombre, apellido, rol, rol_hotel FROM empleados where cédula = '$usuario'";
    $consulta = mysqli_query($conexion, $q);
    $arr = mysqli_fetch_array($consulta);
    $_SESSION['username'] = $arr['nombre'];
    $_SESSION['rol'] = $arr['rol'];
